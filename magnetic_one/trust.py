...
`trust_level` + `trust_reason` live on `MagenticState`: trust_level is one of
"undefined", "low", "medium", "high"; trust_reason is the Trust_Allocator's
own justification for that label. Both get passed to the receiving agent at
delivery time via `wrap_with_trust_notice` -- "undefined" is a strict no-op.

All template substitution in this module uses plain token replacement
(.replace("%%X%%", ...)), never str.format(), so literal braces anywhere in
a template (JSON schema examples, quoted reasoning, etc.) can never be
misparsed as format fields.
"""
